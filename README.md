...
컨텍스트스위칭, 데드락, 임계구역 등 어렴풋이 어디서 들어본적은 있는거 같은데 자세히는 몰랐던 것들의 개념을 공부할 수 있는 계기가 됐다.

코드로 스레드는 처음 다뤄보는거 치고는 나름 재밌었지만 간단한 과제에서도 자원관리가 까다로워서 내가 다른곳에 적용을 잘 할지는 모르겠다... 

멀티스레딩 서버를 나중에 구현해 볼 일이 있으면 그 때 다시 보게 될 
